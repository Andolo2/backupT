            ...
            $getClasses.data | ForEach-Object {
                ...
                if ($_.activityType -eq 'Undervisning') {
                    ...
                    if (-not [string]::IsNullOrEmpty($_.groups[0].id)) {
                        ...
                        if(-not $global:matchToRoster.ContainsKey($_.groups[0].id)){
                            $global:matchToRoster[($_.groups[0].id)] = $classObject
                        }
        ...
        try {
            ...
            $getCourses.data | ForEach-Object {
                ...
                if ($_.activityType -eq 'Undervisning' -and $_.groups[0].id -ne $null) {
                    ...
                    $courseObject.course_name = $_._embedded.syllabus.displayName
...
<# function GetRoosters {
    ...
    foreach ($schoolType in $schoolTypes) {
        try {
            ...
            # If an error occurs during REST API call, catch the exception and output an error message
            Write-Host "An error occurred while fetching roosters for school type $schoolType $($_.Exception.Message)"
            continue
        }
        ...
        try {
            ...
            $GetRooster.data | ForEach-Object {
                $id = $_.id
                $MatchClass = $global:matchToRoster[$id]
                
                if($MatchClass){
                    foreach ($membership in $_.groupMemberships) {
                        $personId = $membership.person.id
                        $roosterId = "$($MatchClass.class_id)_$($personId)"
    
                        # Create a new instance of the Roosters class
                        $roosterObject = [Roosters]::new()
                        $roosterObject.roster_id = $roosterId
                        
                        # Check if $MatchClass.class_id is available, if not, set $id
                        if($MatchClass.class_id){
                            $roosterObject.class_id =  $MatchClass.class_id
                        } else {
                ...
                if ($_.activityType -eq 'Undervisning') {
                    ...
                    if ($_.groups -ne $null -and $_.groups.Count -gt 0 -and $_._embedded.groups[0] -ne $null) {
                        ...
                        foreach ($membership in $_._embedded.groups[0].groupMemberships) {
                            ...
                            $matchClassID = "$($groupid)_$($displayname)"

                            $checkID = $global:classDictonary["$($_.groups[0].id)_$($_.displayName)"]
                            

                            if ($checkID) {

                                if($global:studentDictonary.Keys -eq $($membership.person.id)){
                                    $roosterId = "$($groupid)_$($displayname)_$($membership.person.id)"


                                    $roosterObject = [Roosters]::new()
                                    $roosterObject.roster_id = $roosterID
                                    $roosterObject.class_id = $checkID.class_id
...
Write-Host "Courses   : $($global:courseDictionary.Count)"
 ...
 $global:locationDictonary.Values | Export-Csv -Encoding utf8 -Delimiter ";" -Path "$WorkingFolder\New_Locations.csv" -NoTypeInformation -Force
$global:staffDictonary.Values | Export-Csv -Encoding utf8 -Delimiter ";" -Path "$WorkingFolder\New_Staff.csv" -NoTypeInformation -Force
$global:classDictonary.Values | Export-Csv -Encoding utf8 -Delimiter ";" -Path "$WorkingFolder\New_Classes.csv" -NoTypeInformation -Force
$global:roosterDictonary.Values | Export-Csv -Encoding utf8 -Delimiter ";" -Path "$WorkingFolder\New_Rooster.csv" -NoTypeInformation -Force
$global:courseDictionary.Values | Export-Csv -Encoding utf8 -Delimiter ";" -Path "$WorkingFolder\New_Courses.csv" -NoTypeInformation -Force
$global:studentDictonary.Values | Export-Csv -Encoding utf8 -Delimiter ";" -Path "$WorkingFolder\New_Student.csv" -NoTypeInformation -Force #>
