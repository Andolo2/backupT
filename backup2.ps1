...
$OrganizationVersion = "Organization/v10/Organization"
$OfferingVersion = "Offering/v2/Offering"
$OutcomeVersion = "v5"
...
LogWrite "** $(Get-Date) _____________________ Script begins **"
    ...
    foreach ($SchoolType in $SchoolTypes.Keys) {
        ...
        for ($i = 0; $i -le 9; $i++) {
            ...
            try {
                ...
                Write-Verbose "running process for year $($year)" -Verbose
                
            }
            catch {
                
                LogWrite "Error retrieving data for year $year $_"
                continue
            }
        }
...
function Process-MembershipsAndGroups($ContentOrganizations) {
    foreach ($membership in $ContentOrganizations.enterprise.membership) {
        foreach ($member in $membership.member | Where-Object { $_.role.roletype -eq "Student" -and $_.sourcedid.id -notlike "CG{*" -and $_.role.extension.placement }) {
            ...
            if (-not $global:HistoricEntmembers.ContainsKey($memberId.ToString())) {
                try {
    ...
    if ($ContentOrganizations.enterprise) {
        foreach ($y in $ContentOrganizations.enterprise.membership) {
            foreach ($member in $y.member) {
                # Check if the member has a placement
                if ($member.role.roletype -eq "Student" -and $member.sourcedid.id -notlike "CG{*" -and $member.role.extension.placement) {
                    $memberId = $member.sourcedid.id.Trim('{}')
    
                    # Create or update HistoricPlacement hashtable with composite key
                    $begin = $member.role.extension.placement.begin
                    $compositeKey = "$memberId|$begin"
    
                    if (-not $global:HistoricPlacement.ContainsKey($compositeKey)) {
                        $global:HistoricPlacement.Add($compositeKey, $member)
                    }
                }
            }
        }
    ...
    if ($ContentOrganizations.enterprise) {
        ...
        foreach ($p in $ContentOrganizations.enterprise.person) {
            ...
            if ($pdValue -and -not $global:HistoricEntPersonsPiD.ContainsKey($pdValue)) {
                # Add the PD value to the dictionary if it doesn't already exist
    ...
    if ($ContentOrganizations.enterprise) { # Gather all groups that are type "Class"
    foreach ($c in $ContentOrganizations.enterprise.group) {
        if ($c.grouptype.typevalue.'#text' -eq "Class") {
            $groupId = $c.sourcedid.id
            # $begin = $c.timeframe.begin 

            

            if (-not $global:HistoricGroups.ContainsKey($groupId)) {
                $global:HistoricGroups.Add($groupId, $c)
            }
        }
        else {
            continue
    ...
    if ($ContentOrganizations.enterprise) {
        foreach ($memberships in ($ContentOrganizations.enterprise.membership)) {
            ...
            foreach ($member in  ( $memberships.member | where { $_.role.roletype -eq "Student" -and $_.sourcedid.id -notlike "CG{*" })) {
                ...
                $courseCode = $member.role.extension.activity.coursecode
                ...
                if ($activity) {
                    ...
                    if ($memberBeginDate -and $memberEndDate) {
                        ...
                        if ($memberBeginDate -le $global:RunDateStart -and $global:RunDateEnd -le $memberEndDate) {
                            ...
                            if (!$global:HistoricEntActivities.ContainsKey("$activityId")) {
                                $global:HistoricEntActivities["$activityId"] = [System.Collections.ArrayList]::new()
                                $global:HistoricEntActivities["$activityId"].Add($activity) | Out-Null
                            }
                            else {
                                $global:HistoricEntActivities["$activityId"].Add($activity) | Out-Null
                            }
                    ...
                    if ($global:EntActivitiesCoursecode.ContainsKey("$activityId|$courseCode")) {
                        
                        continue
                    }
                    else {
                        $global:EntActivitiesCoursecode.add("$activityId|$courseCode", $activity)
                    }
...
function Read-EnterpriseXML($enterprise = $ContentOrganization.enterprise) {
    ...
    if ($enterprise.membership) {
        foreach ($member in $enterprise.membership.member) {
            ...
            if ($member.role.extension.placement) {
                ...
                $memberId = $member.sourcedid.id
                $placementBegin = $member.role.extension.placement.begin
                
    
                
                

                if ($global:EntPlacement.ContainsKey("$memberId|$placementBegin")) {
                    continue
                }
                else {
                    $global:EntPlacement.add("$memberId|$placementBegin", $member)
                }
    ...
    if ($enterprise.membership) {
        foreach ($member in $enterprise.membership) {
            if ($member.member.sourcedid.id) {  # check if member ID exists
                $memberId = $member.member.sourcedid.id  # grab the member ID
                $membershipId = $member.sourcedid.id  # grab the membership ID
    
                # Check if the member ID starts with "CG", if so, skip it
                if ($memberId -notlike 'CG*') {
                    if (-not $global:EntGroupMembership.ContainsKey($memberId)) {
                        $global:EntGroupMembership.Add($memberId, $membershipId)
                    }
                }
            }
        }
 ...
 if ($enterprise.group) {
    foreach ($c in $enterprise.group) {
        ...
        if ($c.grouptype.typevalue.'#text' -eq "Class") {
            ...
            $sourceId = $c.sourcedid.id
            # $begin = $c.timeframe.begin
            
            
            if($global:EntClasses.ContainsKey("$($sourceId)")){
                continue
            }else {
                $global:EntClasses.Add("$($sourceId)", $c)
            }
...
function FormatTeacherName {
    ...
    if($fullName) {
        # Split the full name into given names and the last name
        $Parts = $FullName.Split(',')
        $GivenNames = $Parts[1].Trim() -split '\s+'
        $LastName = $Parts[0].Trim() -split '\s+'

        # Remove whitespace and concatenate the names
        $FormattedName = ($GivenNames -join '') + ($LastName -join '')
    } else {
        $formattedName = ""
    }
...
$global:HistoricEntActivities = [System.Collections.Hashtable]::new()
$global:EntActivitiesCoursecode = [System.Collections.Hashtable]::new()
$global:HistoricPlacement=[System.Collections.Hashtable]::new()
...
$global:EntGroupMembership = [System.Collections.Hashtable]::new()
...
foreach ($SchoolType in $SchoolTypes.Keys) {
    ...
    LogWrite "Entering loop at ** $(Get-Date) **"
    ...
    LogWrite "set $global:RunDateStart &  $global:RunDateEnd  "
            ...
            :L3_ClassMember foreach ($L3_ClassMember in $ClassMembers) {
                ...
                if ($ElevExportDictionary.ContainsKey("$( $newElev.Personnummer )|$( $newElev.Klass )")) {
                    ...
                    foreach ($activity in $ForeCastmatches) {
                        ...
                        $GetActivePlacement = $global:EntPlacement["$($activity.MemberID)|$($activity.begin)"]
                        
                        if ($GetActivePlacement) { # Check placement for Schoolyear
                            $SetSchoolYear = $GetActivePlacement.role.extension.placement.schoolyear
                        }
                        else {
                            $GetSchoolYearByClass = $global:EntClasses[$newElev.KlassGUID] # Check group for schoolyear
                            if ($GetSchoolYearByClass) {
                                $SetSchoolYear = $GetSchoolYearByClass.extension.schoolyear
                            }
                            else {
                                $fallback = $global:Entmembers[$activity.MemberID] # Set a schoolyear based on range of dates
                                $activityBegin = [datetime]::Parse($activity.begin)
                                $placementBegin = [datetime]::Parse($fallback.role.extension.placement.begin)
                        
                                if ($activityBegin.Year -eq $placementBegin.Year) {
                                    $SetSchoolYear = $fallback.role.extension.placement.schoolyear
                                } else {
                                    $SetSchoolYear = '' #  NO value found.
                                }
                            }
                        }
                        ...
                        $NewSubjectForecast.LåstBetyg = "False"
                        $NewSubjectForecast.AktivitetensÅrskurs = $SetSchoolYear
                        ...
                        if ($BetygExportDictonary.ContainsKey("$($NewSubjectForecast.PersonnummerElev)|$($NewSubjectForecast.ÄmneKurs)")) {
                            continue
                        }
                        else {
                            $BetygExportDictonary.Add("$($NewSubjectForecast.PersonnummerElev)|$($NewSubjectForecast.ÄmneKurs)", $NewSubjectForecast)
                        }
                    }  
                    ...
                    foreach ($HistCourses in $PlannedHistoryOutcome) {
                        ...
                        if ($HistCourses.Id) {
                            ...
                            $Getstudentdata = $global:HistoricEntPersonsPiD[$HistCourses.Id] # Get the MemberId
                            
                            $GetmemberId = $Getstudentdata.sourcedid.id

                            $trimmedMemberId = $GetmemberId.Trim('{}')
                            
                            
                            $GetHistoricActivity = $global:EntActivitiesCoursecode["$trimmedMemberId|$($HistCourses.CourseTypeCode.coursecode)"] # Match outcome with <activity> from ORG
                            ...
                            $GethistoricPlacement = $global:HistoricPlacement["$($trimmedMemberId)|$($GetHistoricActivity.begin)"]

                            
                            if ($GethistoricPlacement) {
                               
                                $SetHistSchoolYear = $GethistoricPlacement.role.extension.placement.schoolyear
                            } 

                           
                            if(-not $GetHistoricActivity){

                                    "$($trimmedMemberId)|$($HistCourses.CourseTypeCode.coursecode)" | Out-File -FilePath "output.txt" -Append
                                    $GetHistMembershipID = $global:HistoricEntmembers[$GetmemberId]

                                    $GetHistSchoolyearByGroup = $global:HistoricGroups[$GetHistMembershipID.MembershipID]
    
                                    if($GetHistSchoolyearByGroup){
                                        $timeframeBegin = [datetime]::ParseExact($GetHistSchoolyearByGroup.timeframe.begin, 'yyyy-MM-dd', $null)
                                        $timeframeEnd = [datetime]::ParseExact($GetHistSchoolyearByGroup.timeframe.end, 'yyyy-MM-dd', $null)
                                    
                                        $gradeOutcomeDate = [datetime]::Parse($HistCourses.GradeOutcome.Date)
                                    
                                        # Extract the year components and compare only the first four digits
                                        $timeframeBeginYear = $timeframeBegin.ToString('yyyy').Substring(0, 4)
                                        $timeframeEndYear = $timeframeEnd.ToString('yyyy').Substring(0, 4)
                                        $gradeOutcomeYear = $gradeOutcomeDate.ToString('yyyy').Substring(0, 4)
                                    
                                        if ($gradeOutcomeYear -ge $timeframeBeginYear -and $gradeOutcomeYear -le $timeframeEndYear) {
                                            "$($HistCourses.CourseTypeCode.coursecode)`r`n $($HistCourses.Id)`r`n$($HistCourses.Name)`r`n$($GetHistSchoolyearByGroup.sourcedid.id)`r`n$($GetHistSchoolyearByGroup.extension.schoolyear)`r`n_______________" | Out-File -FilePath "txt.txt" -Append
                                        } else {
                                            # Do nothing or add any additional actions you want to take for the else condition
                                        }
                                    }
                                    ...
                                    $historicOutcome.LåstBetyg = "False"# $HistCourses.GradeOutcome.trialperformed
                                    $historicOutcome.AktivitetensGUID = $HistCourses.CourseData.UnitId
                                    $historicOutcome.AktivitetensÅrskurs = "1"
                                    $historicOutcome.GruppGUID = "" 
                                
                            } else {    
                                $historicOutcome.PersonnummerElev = Format-PersonnummerElev -Id $HistCourses.Id
                                $historicOutcome.GruppNamn = "" # $HistCourses.CourseData.SchoolName | tomt på historsiak betyg.
                                $historicOutcome.ÄmneKurs = $GetHistoricActivity.coursecode
                                $historicOutcome.Kursnamn = $HistCourses.CourseTypeCode.CourseName
                                $historicOutcome.Poäng = $HistCourses.CourseTypeCode.CoursePoints
                                $historicOutcome.Startdatum = $GetHistoricActivity.begin
                                $historicOutcome.Slutdatum = $GetHistoricActivity.end
                                $historicOutcome.Betyg = $HistCourses.GradeOutcome.Grade
                                $historicOutcome.Akttyp = ""
                                $historicOutcome.Hur = ""
                                $historicOutcome.Period = FormatPeriod -inputTimestamp $HistCourses.GradeOutcome.Date
                                ...
                                $historicOutcome.LåstBetyg = "False" #$HistCourses.GradeOutcome.trialperformed
                                $historicOutcome.AktivitetensGUID = $HistCourses.CourseData.UnitId
                                $historicOutcome.AktivitetensÅrskurs = $SetHistSchoolYear
                                $historicOutcome.GruppGUID = ""
                            }
                        ...
                        if ($BetygExportDictonary.ContainsKey("$($historicOutcome.PersonnummerElev)|$($historicOutcome.ÄmneKurs)")) { 
                         
                            continue
                        }
                        else {
                           
                            $BetygExportDictonary.Add("$($historicOutcome.PersonnummerElev)|$($historicOutcome.ÄmneKurs)", $historicOutcome)
                        }
                    }   
...
<# function Check-for-Dubplicates_TEST {
    param (
        [string]$FilePath
    )
    ...
    Get-Content $FilePath | ForEach-Object {
        ...
        if (-not [string]::IsNullOrWhiteSpace($line)) {
            ...
            if ($parts.Count -eq 2) {
                $pids = $parts[0]
                $name = $parts[1]
                if ($pidNames.ContainsKey($pids)) {
                    if ($pidNames[$pids] -ne $name) {
                        [PSCustomObject]@{
                            PID = $pids
                            Name1 = $pidNames[$pids]
                            Name2 = $name
...
$FilePath = "txt.txt"
$inconsistentPids = Check-Names -FilePath $FilePath

if ($inconsistentPids -ne $null) {
    Write-Host "The following Person IDs have inconsistent names:"
    $inconsistentPids | ForEach-Object {
        Write-Host "PID: $($_.PID)"
        Write-Host "Name 1: $($_.Name1)"
        Write-Host "Name 2: $($_.Name2)"
        Write-Host ""
    }
} else {
    Write-Host "All Person IDs have consistent names."
}
  #>
